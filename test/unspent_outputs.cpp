...
BOOST_AUTO_TEST_CASE(unspent_outputs__remove2__remove_one_output__expected_outputs)
{
    ...
    unspent_outputs cache(42);
    cache.add(tx1, 0);
    cache.add(tx2, expected_height);
    BOOST_REQUIRE_EQUAL(cache.size(), 2u);

    bool out_coinbase;
    size_t out_height;
    chain::output out_value;
    BOOST_REQUIRE(cache.get(out_value, out_height, out_coinbase, { tx1.hash(), 0 }, max_size_t));
    BOOST_REQUIRE(cache.get(out_value, out_height, out_coinbase, { tx1.hash(), 1 }, max_size_t));
    BOOST_REQUIRE(cache.get(out_value, out_height, out_coinbase, { tx2.hash(), 0 }, max_size_t));
    BOOST_REQUIRE(cache.get(out_value, out_height, out_coinbase, { tx2.hash(), 1 }, max_size_t));
...
BOOST_AUTO_TEST_CASE(unspent_outputs__get__two_capacity_1__size_1_expected)
{
    ...
    unspent_outputs cache(1);
    cache.add(tx1, expected_height);
    BOOST_REQUIRE_EQUAL(cache.size(), 1u);

    bool out_coinbase;
    size_t out_height;
    chain::output out_value1;
    BOOST_REQUIRE(cache.get(out_value1, out_height, out_coinbase, { tx1.hash(), 1 }, max_size_t));
    BOOST_REQUIRE_EQUAL(out_coinbase, false);
    BOOST_REQUIRE_EQUAL(out_height, expected_height);
    ...
    BOOST_REQUIRE(cache.get(out_value2a, out_height, out_coinbase, { tx2.hash(), 1 }, max_size_t));
    BOOST_REQUIRE(out_value2a.is_valid());
    BOOST_REQUIRE_EQUAL(out_value2a.value(), expected2a);

    chain::output out_value2b;
    BOOST_REQUIRE(cache.get(out_value2b, out_height, out_coinbase, { tx2.hash(), 2 }, max_size_t));
