...
code CLASS::get_fault() const NOEXCEPT
{
    ...
    if ((ec = header_head_.get_fault())) return ec;
    if ((ec = header_body_.get_fault())) return ec;
    if ((ec = input_head_.get_fault())) return ec;
    if ((ec = input_body_.get_fault())) return ec;
    if ((ec = output_head_.get_fault())) return ec;
    ...
    if ((ec = schnorr_head_.get_fault())) return ec;
    ...
    if ((ec = prevout_head_.get_fault())) return ec;
    if ((ec = prevout_body_.get_fault())) return ec;
    if ((ec = validated_bk_head_.get_fault())) return ec;
    ...
    if ((ec = address_head_.get_fault())) return ec;
