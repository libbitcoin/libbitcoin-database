...
/// SoA aggregate hash table: one keyed spine with satellite record columns.
/// Column zero is the keyed spine (link/key/value rows searchable from the
/// hash head). Trailing Widths are satellite records sharing the spine's row
/// count, with allocation unified across all columns (one lock). One storage
/// backs all columns, so any column accessor guards the body against remap.
...
template <class Link, class Key, size_t RowSize, size_t CellSize,
    size_t... Widths>
class hashmaps
{
public:
    DELETE_COPY_MOVE_DESTRUCT(hashmaps);
    ...
    /// Aggregate body bytes across all columns.
    size_t body_size() const NOEXCEPT;

    /// Aggregate body byte capacity across all columns.
    ...
    /// Allocate count records at returned link (follow with set|put).
    ...
    /// Get element at link, false if deserialize error.
    /// Iterator must not be terminal, must be guarded by caller.
    template <typename Element, if_equal<Element::size, RowSize> = true>
...
private:
    ...
    using body = database::bodys<Link, Key, RowSize, Widths...>;
...
template <typename Spine, typename... Columns>
using hash_maps = hashmaps<typename Spine::link, typename Spine::key,
    Spine::size, Spine::cell, Columns::width...>;

} // namespace database
} // namespace libbitcoin

#define TEMPLATE template <class Link, class Key, size_t RowSize, \
    size_t CellSize, size_t... Widths>
#define CLASS hashmaps<Link, Key, RowSize, CellSize, Widths...>
#define ELEMENT_CONSTRAINT template <class Element, \
    if_equal<Element::size, RowSize>>

#include <bitcoin/database/impl/primitives/hashmaps.ipp>
