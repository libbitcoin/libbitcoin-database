...
#ifndef LIBBITCOIN_DATABASE_INTERPROCESS_FILE_LOCK_HPP
#define LIBBITCOIN_DATABASE_INTERPROCESS_FILE_LOCK_HPP
...
inline void *create_file(const std::string& name, unsigned long access, unsigned long creation_flags, unsigned long attributes, boost::interprocess::winapi::interprocess_security_attributes *psec)
{
    for (unsigned int attempt(0); attempt < boost::interprocess::winapi::error_sharing_violation_tries; ++attempt){
        void * const handle = libbitcoin::database::CreateFileUTF8(name, access,
...
(const std::string& name, boost::interprocess::mode_t mode, bool temporary = false)
{
    ...
    return libbitcoin::database::create_file
...
{
#ifdef _MSC_VER
   m_file_hnd = libbitcoin::database::open_existing_file(name, boost::interprocess::read_write);
#else
...
#endif   //LIBBITCOIN_DATABASE_INTERPROCESS_FILE_LOCK_HPP
